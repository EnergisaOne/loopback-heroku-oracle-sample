...
module.exports = function(http) {
	...
	proxy.before(http.Server.prototype, [ 'on', 'addListener' ], function(obj,
		...
		proxy.callback(args, -1, function(obj, args) {
			...
			var latency = 0;
			if (req.headers['x-strongloop-starttime']) {
				var starttime = parseInt(req.headers['x-strongloop-starttime']);
				latency = Date.now() - starttime;
				tiers.sample('latency', { ms: latency });
				var graph = nf.graph = {
					nodes: [
						{ name: req.url },
						{
					...
					links: [
						{ source: 0, target: 1, value: latency }
					]
				};
				var currentNode = nf.currentNode = 1;
			}
			else {
			...
			proxy.after(res, 'end', function(obj, args) {
				...
				try {
					graph.nodes[0].value = timer.ms + latency;
					//console.log('graph: \n%s', util.inspect(graph));
